...
private fun FamilyApp(repo: FamilyRepositoryImpl, store: SessionStore) {
    ...
    var boot by remember { mutableStateOf<AppBoot?>(null) }
    val scope = rememberCoroutineScope()
    LaunchedEffect(Unit) {
        lang = AppStrings.normalize(store.lang())
        val restored = repo.restoreSession()
        boot = AppBoot(
            start = if (restored) "home" else "join",
            familyCode = store.familyCode().orEmpty(),
        )
    }
...
@Composable
private fun FamilyNav(
    repo: FamilyRepositoryImpl,
    strings: Strings,
    boot: AppBoot,
    onToggleLang: () -> Unit,
) {
    ...
    NavHost(navController = nav, startDestination = boot.start) {
        composable("join") {
            val vm = remember {
                JoinViewModel(JoinFamilyUseCase(repo), repo, boot.familyCode)
            }
            JoinScreen(
                viewModel = vm,
                strings = strings,
